...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserGames = async () => {
      try {
        const res = await axios.get(`https://hungnt.backendintern.online/:8800/api/game/publisher/${isLoggedIn.userInfo.id}`, { withCredentials: true });
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('https://hungnt.backendintern.online/:8800/api/user/change-password', { userId, currentPassword, newPassword, confirmNewPassword }, { withCredentials: true });
      if (res.data.success) {
